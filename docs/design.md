...
- **Identity provider unavailable, on the MCP path:** discovery still answers
  ...
  `temporarily_unavailable`; an issued token keeps working while its id_token
  verifies against the cached keys; a refresh fails and the client relinks later.
  The provider's endpoints are read lazily from the browser login's cached
  discovery document, so a provider that is down at start never fails the start.
...
7. **MCP OAuth** — the FastMCP proxy wired with the spike's decisions, owner
   ...
   **Shipped (#192):** `app/auth/mcp_oauth.py` — a `PlamotrackOAuthProxy` over
   ...
   start would fail the start (§5.6); the upstream endpoints are placeholders that
   resolve nowhere until read from `OidcProvider.metadata()` at each entry point,
   and the two `OIDCProxy` hooks that make the **id_token** the verified token are
   reproduced (the full-link test is what noticed the first head verifying the
   opaque upstream access token instead). (c) **The mount requires no OAuth scope**:
   ...
   `auth.mcp_grant_issued` naming the client. (e) **`/revoke` is registered
   unconditionally** and resolves to the provider's endpoint, or to none, on first
   use, so the route table does not depend on the provider's document. (f) The
   ...
   the upstream token is re-validated on every request and refreshed 30 s ahead of
   its expiry. (g) `access_type=offline&prompt=consent` are forwarded to **every**
   ...
   verifier does for a PAT's public id. Not done here: a `resource_metadata`
