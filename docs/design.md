...
7. **MCP OAuth** — the FastMCP proxy wired with the spike's decisions, owner
   ...
   *a NumericDate's range* — ±2^53 (RFC 7493 §2.2), judged on the parsed value before
   ...
   do) — and scheme and authority as the protected-resource document spelled them;
   `authorize` applies it again behind the hand-off, so FastMCP's looser check decides
   nothing (it cannot refuse what this accepted). *The recognised fields* —
