...
7. **MCP OAuth** — the FastMCP proxy wired with the spike's decisions, owner
   ...
   **grant state** (Codex #212 round 1, f3): carried under the proxy's own signature
   ...
   verifies it in full and requires the same owner. (e) **`/revoke` is registered
   ...
   *its* refresh token, and `auth.mcp_grant_revoked` names the client. (f) The
   ...
   second redeemer reads the first's deletion and gets `invalid_grant`. (g) `access_type=offline&prompt=consent` are forwarded to **every**
