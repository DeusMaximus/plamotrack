...
11. **Availability bounds from the 0.3.0 scan (#221)** — the owner's full-repository
    ...
    stream of fresh addresses hold the owner out — so a browser that has signed in
    before is admitted from a reserved bucket (Codex #222 round 1, f1), and a
    brand-new browser during a sustained distributed flood is explicitly *not*
    bounded by the app: the operator's ingress is; the cheap paths — the setup token,
