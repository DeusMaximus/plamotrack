...
7. **MCP OAuth** — the FastMCP proxy wired with the spike's decisions, owner
   ...
   *request* defaults to `client_secret_post`, which is why the method is
   ...
   valid, authenticated revocation into `400 invalid_request`. **Who owns what, per
   endpoint** (the audit the round asked for; a documented extension point says
   where code runs, not what surrounds it): the **discovery documents** — ours
