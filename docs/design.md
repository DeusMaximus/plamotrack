...
7. **MCP OAuth** — the FastMCP proxy wired with the spike's decisions, owner
   ...
   *a NumericDate's range* — ±2^53 (RFC 7493 §2.2), judged on the parsed value before
   ...
   both endpoints. *The recognised fields* —
   ...
   **inline key set's shape** — ours (`with_usable_inline_keys`), the key's conversion
   and the signature FastMCP's; the **discovery documents** — ours
