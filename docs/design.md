...
11. **Availability bounds from the 0.3.0 scan (#221)** — the owner's full-repository
    ...
    cap is checked before the write and a race can overshoot it by the concurrent
    registrations nginx admits, which is the bound accepted; a CIMD lookup at the
    cap is an unknown client rather than a 500; the collection's cap refuses new
    keys only, so a full collection never breaks a client that exists; the
    verification budget can delay the owner by seconds under a distributed flood
    and is the price of bounding the worker's Argon2 work — the alternative, no
    instance-wide bound, would let a flood of fresh addresses saturate the one worker.
