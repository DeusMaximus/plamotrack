...
7. **MCP OAuth** — the FastMCP proxy wired with the spike's decisions, owner
   ...
   *its* refresh token, and `auth.mcp_grant_revoked` names the client. And the
   ...
   truthfully). Registration is open, so a downstream secret would be minted to
   whoever asks and would authenticate nothing PKCE and the rotating refresh token
   do not; the authority is the owner's upstream login and the grant machinery. The
   ...
   *request* defaults to `client_secret_post`, which is why the method is
   ...
   §3) and usable once; the reviewed head's `/revoke` used the plain SDK
   authenticator, which refused the method the client had linked with. **The wire
   forms are the RFCs'**: `client_id` in the form on both endpoints for every kind
   (RFC 6749 §3.2.1 requires it of a public client; RFC 7523 §2.2 permits it beside
   an assertion), no secret from a public client (one sent anyway is ignored, as the
   ...
   the form beside the header — is unreachable. **Who owns what, per endpoint** (the
   audit the round asked for; a documented extension point says where code runs,
   not what surrounds it): `/register` — validation the SDK's handler (metadata
   model, grant and response types, scopes), persistence FastMCP's `register_client`
   under the proxy's override that makes the object public before it is stored or
   returned, the response the SDK's from that same object (pinned by the contract
   suite: a future handler that copied the object would advertise a secret again),
   the non-JSON body ours (`guard_registration_body`); `/authorize` — client lookup
