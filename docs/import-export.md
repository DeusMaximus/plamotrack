...
| `quantity` | How many of this kit. Blank = 1, at most 1,000 per row. You get that many kits in your collection either way — whether the row names a retailer or not — so a bigger haul goes on more than one row (one import creates at most 10,000 kits in total this way — see Limits). |
