...
| **Cursor (Grok 4.6, SpaceXAI)** | Back since 2026-09-07 (owner's call); fix rounds of a size it can hold whole | Retired 2026-08-28 for the 256K ceiling; returned for #219 (GO + 1 P3): wrote the feature contract before reading the author's plan, found the P3 in the gap between its list and the coverage record — a collection the branch itself classified as a grant's, purged unread — and named the remedy's wrong half (the record's top level vs `idp_tokens`). Re-measured the control and all 16 mutants exactly, re-sampled the race kill 3/3, and listed what it left unexamined. The ceiling still applies: not for the very large PRs. |
