...
```bash
cd backend && uv run python mutation_test.py          # every case — ~18 min at 176 cases on
                                                      # the primary dev Mac (18m08s measured
                                                      # at the #143 fold-in, 25/08/2026;
...
- **On `main` at the time of writing: 176 cases over thirteen files** — #86's
  ...
  in strt-7's label; that branch also re-anchored adv-7) and #143 (`rcv-`).
