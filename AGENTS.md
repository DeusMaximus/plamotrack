...
require the owner's explicit instruction. v0.5.1-alpha is the first release to carry
these assets. A source checkout always uses the build override below: `.env.example`
selects it through `COMPOSE_FILE`, and an `.env` from v0.5.0-alpha or earlier needs
those two lines added before its next `up`, or Compose pulls the never-published
`:unreleased` image and stops with `denied`.
