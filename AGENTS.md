...
7.2. **Export reads one snapshot** — `await begin_read_snapshot(session)` from
   ...
   `REPEATABLE READ READ ONLY`, so every table comes from the same instant: one
   statement per table under the default `READ COMMITTED` is one snapshot per
   table, and a write landing between two of them writes an archive whose files
   contradict each other — a kit whose order line no CSV in the same zip contains
   (#48). This is why reads still don't take the write gate: an export must never
   queue behind a writer, or the reverse. Because it covers the whole transaction
   and Postgres then refuses every write in it, put it only where the read *is*
   the unit of work. Never on a helper a write path also calls — `plan_import` is
