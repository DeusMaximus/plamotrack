...
| **Phone and tablet layout** | 🔨 Milestone 6.6, in progress — built so far: a bottom tab bar on a phone, an icon rail on a tablet or in a narrow window (the desktop layout is unchanged from 1280 px), touch-sized controls, and Add to Home Screen (an icon and a standalone window; no offline mode). Still to come: the list pages still show their tables on a phone, scrolling sideways inside their boxes, and the dialogs are not yet full-screen sheets (`docs/design.md` §13.7) |
